...
export default function ResearchLaboratories() {
  ...
  const [labs, setLabs] = useState<LaboratoryListItem[]>([]);
  ...
  const fetchLabs = (s: number, e: number) => {
    setLoading(true);
    setError(null);
    getAllLaboratories(s, e)
      .then((res) => {
        if (res && typeof res === "object" && "laboratories" in res) {
          setLabs(res.laboratories ?? []);
          setTotal(res.total ?? 0);
        } else {
          setError("Laboratoriyalar yüklənərkən xəta baş verdi.");
          setLabs([]);
          setTotal(0);
        }
      })
  ...
  const handleDelete = async (id: number) => {
    ...
    if (result === "SUCCESS") {
      ...
      Swal.fire({ icon: "error", title: "Xəta", text: "Laboratoriya tapılmadı", showConfirmButton: false, timer: 1500 });
    } else {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end">
        <Link to="/research-laboratories/new" className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-sm font-medium">
          Yeni Laboratoriya
        </Link>
      </div>
      <div className="overflow-hidden rounded-2xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm">
        <div className="flex items-center px-5 py-3 bg-gray-50/80 dark:bg-gray-800/50 border-b border-gray-100 dark:border-gray-800">
          <p className="text-[11px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500" style={{ width: "10%" }}>Şəkil</p>
          <p className="text-[11px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500" style={{ width: "40%" }}>Laboratoriya adı</p>
          <p className="text-[11px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500" style={{ width: "30%" }}>Kafedra</p>
        ...
        {loading ? (
          <div>
            {[...Array(PAGE_SIZE)].map((_, idx) => (
              <div key={idx} className="flex items-center px-5 py-4 border-b border-gray-50 dark:border-gray-800 last:border-b-0 animate-pulse">
                <div className="h-10 w-10 bg-gray-100 dark:bg-gray-800 rounded-lg" style={{ width: "8%" }}></div>
                <div className="h-3 bg-gray-100 dark:bg-gray-800 rounded-full ml-4" style={{ width: "38%" }}></div>
                <div className="h-3 bg-gray-100 dark:bg-gray-800 rounded-full ml-4" style={{ width: "28%" }}></div>
          ...
          labs.map((lab) => (
            <div key={lab.id} className="flex items-center px-5 py-3.5 border-b border-gray-50 dark:border-gray-800 last:border-b-0 hover:bg-gray-50/60 dark:hover:bg-gray-800/40 transition-colors duration-150">
              <div style={{ width: "10%" }}>
                {lab.image_url ? (
                  <img src={getImageUrl(lab.image_url)} alt="" className="h-10 w-10 rounded-lg border border-gray-200 dark:border-gray-700 object-cover" />
                ) : (
                  <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-gray-100 text-xs text-gray-400 dark:bg-gray-800">—</div>
                )}
              </div>
              <p className="text-sm font-medium text-gray-700 dark:text-gray-200" style={{ width: "40%" }}>{lab.title || "—"}</p>
              <p className="text-sm text-gray-500 dark:text-gray-400" style={{ width: "30%" }}>{cafedraNames[lab.cafedra_code] ?? lab.cafedra_code}</p>
              <div className="flex justify-end items-center gap-1" style={{ width: "20%" }}>
                <Link to={`/research-laboratories/${lab.id}`}>
                ...
                <button type="button" className="p-1.5 rounded-lg text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors" onClick={() => handleDelete(lab.id)} disabled={deletingId === lab.id} title="Sil">
                  {deletingId === lab.id ? <CircularProgress size={16} sx={{ color: "currentColor" }} /> : <DeleteIcon sx={{ fontSize: 18 }} />}
                </button>
              </div>
            </div>
          ))
          ...
          <div className="flex flex-col items-center justify-center py-16">
            ...
            <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">Laboratoriya yoxdur</p>
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Hələ heç bir tədqiqat laboratoriyası əlavə edilməyib</p>
          </div>
        )}
      </div>
