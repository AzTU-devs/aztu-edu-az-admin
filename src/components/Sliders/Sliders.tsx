    ...
    <div className="space-y-4">
      <div className="overflow-hidden rounded-2xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm">
        <div className="flex items-center px-5 py-3 bg-gray-50/80 dark:bg-gray-800/50 border-b border-gray-100 dark:border-gray-800">
          ...
          <p className="text-[11px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500" style={{ width: "8%" }}>Sıra</p>
          <p className="text-[11px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500" style={{ width: "35%" }}>Təsvir</p>
          <p className="text-[11px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500" style={{ width: "25%" }}>URL</p>
        ...
        {loading ? (
          <div>
            {[...Array(PAGE_SIZE)].map((_, idx) => (
              <div key={idx} className="flex items-center px-5 py-4 border-b border-gray-50 dark:border-gray-800 last:border-b-0 animate-pulse">
                ...
                <div className="h-3 bg-gray-100 dark:bg-gray-800 rounded-full ml-1" style={{ width: "7%" }}></div>
                <div className="h-3 bg-gray-100 dark:bg-gray-800 rounded-full ml-3" style={{ width: "33%" }}></div>
                <div className="h-3 bg-gray-100 dark:bg-gray-800 rounded-full ml-3" style={{ width: "23%" }}></div>
                <div className="h-5 bg-gray-100 dark:bg-gray-800 rounded-full ml-3" style={{ width: "8%" }}></div>
                <div className="flex justify-end gap-1.5 ml-3" style={{ width: "16%" }}>
          ...
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
            <SortableContext items={sliders.map((s) => s.slider_id.toString())} strategy={verticalListSortingStrategy}>
              {sliders.map((slider) => (
                <SortableItem key={slider.slider_id} id={slider.slider_id.toString()}>
                  {({ attributes, listeners }) => (
                    <div className="flex items-center px-5 py-3.5 border-b border-gray-50 dark:border-gray-800 last:border-b-0 hover:bg-gray-50/60 dark:hover:bg-gray-800/40 transition-colors duration-150">
                      ...
                      <p className="text-sm font-semibold text-gray-500 dark:text-gray-400" style={{ width: "8%" }}>{slider.display_order}</p>
                      <p className="text-sm font-medium text-gray-700 dark:text-gray-200" style={{ width: "35%" }}>{truncate(slider.desc, 50)}</p>
                      <p className="text-xs text-blue-500 dark:text-blue-400 truncate" style={{ width: "25%" }}>{truncate(slider.url, 35)}</p>
                      <div style={{ width: "10%", display: "flex", justifyContent: "center" }}>
                        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-semibold ${slider.is_active ? "bg-emerald-50 text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-400" : "bg-red-50 text-red-600 dark:bg-red-900/20 dark:text-red-400"}`}>
                          <span className={`h-1.5 w-1.5 rounded-full ${slider.is_active ? "bg-emerald-500" : "bg-red-500"}`} />
                      ...
                      <div className="flex justify-end items-center gap-1" style={{ width: "17%" }}>
                        ...
                        <button type="button" className="px-2.5 py-1 text-[11px] font-semibold rounded-lg text-brand-600 bg-brand-50 hover:bg-brand-100 dark:bg-brand-900/20 dark:text-brand-400 dark:hover:bg-brand-900/30 transition-colors" onClick={() => handleChangeOrder(slider.slider_id, slider.display_order)}>Sıra</button>
                        <button type="button" className="p-1.5 rounded-lg text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors" onClick={() => handleDelete(slider.slider_id)} disabled={deletingId === slider.slider_id} title="Sil">
                          {deletingId === slider.slider_id ? <CircularProgress size={16} sx={{ color: "currentColor" }} /> : <DeleteIcon sx={{ fontSize: 18 }} />}
                        </button>
                      </div>
                    </div>
                  )}
          ...
          <div className="flex flex-col items-center justify-center py-16">
            ...
            <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">Slayder yoxdur</p>
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Hələ heç bir slayder əlavə edilməyib</p>
          </div>
        )}
      </div>
